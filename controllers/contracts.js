...
const postDeclineContractProposal = (req, res, next) => {
    ...
    try {
        ContactProposal.deleteOne({ id: requestId }, function (err) {
            ...
            return res.status(204).json({ "responseMessage": "contact proposal deleted" })
        })
    } catch (err) {
        console.log(err);
        return res.status(500).json({ "err": err.message })
...
const postContractProposal = (req, res, next) => {
    ...
    try {
        let firstUserId = req.userId;
        let secondUserId = req.body.to;
        //TODO improvement functionality : add checks for if users are already friends 
